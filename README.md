...
- `k`: [0.3, 1.0, 2.0]
- **网格大小**: 48 configs × 3 repeats = 144 runs/实验
...
### 结果目录
- `result/comprehensive_experiments/tuning_XLasso-PreDecomp_exp{1-7}_{timestamp}/`
- 完成时间: 2026-03-24 20:11:33
...
| Algorithm | exp1 | exp2 | exp4 | exp5 | exp6 | exp7 |
|-----------|------|------|------|------|------|------|
| **PreDecomp** | **3.18** | **2.31** | **2.25** | **1.17** | **1.28** | **2.62** |
| XLasso-Stage2 | 4.02 | 2.92 | 4.07 | 2.20 | 2.70 | 4.36 |
| Lasso | 4.17 | 3.82 | 3.57 | 2.48 | 2.96 | 5.24 |
| UniLasso | 4.21 | 3.59 | 3.44 | 2.50 | 3.03 | 5.23 |
| AdaptiveLasso | 196.08 | 64.12 | 169.77 | 2.27 | 11.39 | 4.82 |
| GroupLasso | 183.97 | 68.69 | 158.40 | 2.48 | 11.98 | 4.85 |
| FusedLasso | 183.83 | 68.72 | 158.69 | 2.48 | 11.98 | 5.48 |

### F1 Score (所有任务)

| Algorithm | exp1 | exp2 | exp3 | exp4 | exp5 | exp6 | exp7 |
|-----------|------|------|------|------|------|------|------|
| **PreDecomp** | 0.36 | 0.58 | **0.99** | 0.96 | 0.00 | 0.80 | **0.40** |
| XLasso-Stage2 | 0.48 | **0.79** | 0.86 | 0.87 | 0.02 | 0.81 | 0.15 |
| Lasso | 0.45 | 0.65 | 0.78 | 0.42 | 0.00 | 0.45 | 0.15 |
| UniLasso | 0.47 | 0.84 | 0.81 | **0.98** | 0.00 | **0.98** | 0.12 |
| AdaptiveLasso | **0.67** | 0.62 | 0.76 | 0.58 | 0.00 | 0.58 | 0.23 |
| GroupLasso | 0.24 | 0.10 | 0.28 | 0.18 | 0.11 | 0.12 | 0.11 |
| FusedLasso | 0.25 | 0.12 | 0.00 | 0.10 | 0.12 | 0.08 | 0.11 |

### 分类任务详情 (exp3 二分类偏移变量)

| Algorithm | Accuracy | AUC | F1 | TPR | FDR |
|-----------|----------|-----|-----|-----|-----|
| **PreDecomp** | **0.993** | 0.995 | **0.992** | **0.983** | **0.000** |
| XLasso-Stage2 | 0.993 | **0.997** | 0.857 | 0.761 | 0.000 |
| Lasso | 0.979 | 0.994 | 0.781 | 0.822 | 0.167 |
| UniLasso | 0.980 | 0.994 | 0.809 | 0.689 | 0.000 |
| AdaptiveLasso | 0.983 | 0.997 | 0.759 | 0.728 | 0.184 |
| GroupLasso | 0.543 | 0.989 | 0.282 | 0.178 | 0.000 |
| FusedLasso | 0.542 | 0.992 | 0.000 | 0.000 | 0.000 |

---

## 各实验最优算法汇总

| Exp | 场景 | 最优MSE算法 | MSE值 | 最优F1算法 | F1值 |
|-----|------|-----------|-------|-----------|------|
| exp1 | 高维成对相关 | **PreDecomp** | 3.18 | AdaptiveLasso | 0.67 |
| exp2 | AR(1)相关 | **PreDecomp** | 2.31 | XLasso-Stage2 | 0.79 |
| exp3 | 二分类偏移 | **PreDecomp** | Acc=0.993 | **PreDecomp** | **0.99** |
| exp4 | 孪生变量 | **PreDecomp** | 2.25 | UniLasso | 0.98 |
| exp5 | 绝对隐身 | **PreDecomp** | 1.17 | Stage2 | 0.02 |
| exp6 | 鸠占鹊巢 | **PreDecomp** | 1.28 | UniLasso | 0.98 |
| exp7 | AR(1)符号雪崩 | **PreDecomp** | 2.62 | **PreDecomp** | 0.40 |
...
### PreDecomp 优势场景
1. **二分类偏移变量 (exp3)**: F1=0.991，显著优于其他算法(+0.13)
2. **孪生变量反符号 (exp4)**: MSE=2.25，所有算法中最优
3. **鸠占鹊巢陷阱 (exp6)**: MSE=1.28，比第二名低52%
4. **所有gaussian回归任务**: MSE均为最优或接近最优

### PreDecomp 劣势场景
1. **绝对隐身陷阱 (exp5)**: F1=0，无法选择变量（MSE虽低但无变量选择能力）
2. **高维成对相关 (exp1)**: F1=0.36，低于AdaptiveLasso(0.67)

### 算法选择建议

| 场景 | 推荐算法 | 原因 |
|------|---------|------|
| 二分类+偏移变量 | **PreDecomp** | F1提升27% |
| 孪生变量对 | **PreDecomp/UniLasso** | MSE/F1最优 |
| 鸠占鹊巢模式 | **PreDecomp** | MSE降低52% |
| 标准稀疏回归 | Lasso/UniLasso | 更简单高效 |
| AR(1)相关结构 | **PreDecomp** | MSE最优 |
| 绝对隐身模式 | 避免PreDecomp | 使用Lasso/Stage2 |
| 未知结构/探索性 | **XLasso-Stage2** | 鲁棒性最好 |

### PreDecomp vs Benchmark 总结
