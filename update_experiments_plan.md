...
## 背景
XLasso (之前: UniLasso) 算法相较于传统方法有两个关键升级：
1. **分组约束升级** - 对高度相关的特征进行分组，促进分组内变量的同时选择/剔除
2. **自适应加权** - 根据单变量显著性调整惩罚项，降低显著变量的惩罚，增加不显著变量的惩罚

> 命名说明：
> - **UniLasso** = 对非负系数做硬约束的原始算法
> - **XLasso** = 带分组约束和自适应加权的新算法（当前算法）
...
├── notebooks/
│   └── all_experiments_comparison.ipynb  # 综合对比笔记本 ✓ (新增)
...
| 方法 | 分组约束 | 自适应加权 | 说明 |
|------|----------|------------|------|
| XLasso (baseline) | ❌ | ❌ | 无升级，普通 Lasso 类似 |
| XLasso + 自适应加权 | ❌ | ✅ | 只看自适应加权效果 |
| XLasso + 分组约束 | ✅ | ❌ | 只看分组约束效果 |
| XLasso (full) | ✅ | ✅ | 完整版算法 |
...
### 阶段四：GLM 扩展实验 - 验证各GLM族下升级仍然有效 ✓ 已完成
...
**`notebooks/all_experiments_comparison.ipynb`** - 一站式运行所有实验，结果内联显示 ✓
- 方便实验探索阶段交互式查看结果
- 自动汇总表格和热力图
- 直接在 notebook 中对比所有场景

## 新建文件列表 ✓ 全部完成

| 路径 | 用途 | 预估行数 | 实际状态 |
|------|------|----------|----------|
| `experiments/__init__.py` | 包初始化 | ~10 行 | ✓ 完成 |
| `experiments/base_experiment.py` | 实验基类 | ~150 行 | ✓ 完成 |
| `experiments/linear_experiment.py` | 线性高斯实验 | ~200 行 | ✓ 完成 |
| `experiments/glm_experiment.py` | 所有 GLM 族实验 | ~200 行 | ✓ 完成 |
| `experiments/nonlinear_experiment.py` | 非线性对比实验 | ~250 行 | ✓ 完成 |
| `experiments/visualization.py` | 共享绘图工具 | ~150 行 | ✓ 完成 |
| `scripts/run_linear_experiment.py` | 线性运行脚本 | ~50 行 | ✓ 完成 |
| `scripts/run_glm_experiment.py` | GLM 运行脚本 | ~50 行 | ✓ 完成 |
| `scripts/run_nonlinear_experiment.py` | 非线性运行脚本 | ~50 行 | ✓ 完成 |
| `notebooks/all_experiments_comparison.ipynb` | 综合探索笔记本 | - | ✓ 新增完成 |
...
✅ **所有计划代码已经编写完成，框架可以正常运行**

已修复的 API 参数不匹配问题：
- 参数名 `use_grouping` → `enable_group_constraint`
- 参数名 `adaptive_weights` → `adaptive_weighting`
- 参数名 `n_lambdas` → `n_lmdas`
- 参数名 `random_state` → `seed`
- `cv_uni` 返回 `UniLassoCVResult`，正确提取 `coefs[best_idx]`
- `model.groups` 是 `List[List[int]]` 正确处理循环

## 验证

实现完成后，我们可以：
1. 用较少重复次数运行每个实验，验证其正确性 ✓（已验证单场景运行正常）
2. 检查所有指标计算正确 ✓
3. 验证图像成功生成 ✓
4. 确保固定随机种子下框架可复现 ✓
