...
"""
实验1：高维成对相关稀疏回归
n=300, p=1000, 变量间成对相关系数0.5
前100个变量系数服从N(0,1)，剩余900个为0
对比5种方法在不同信噪比下的性能
"""
import os
import sys
...
def run_experiment():
    ...
    for repeat in tqdm(range(CONFIG["n_repeats"]), desc="实验重复"):
        ...
        for sigma in tqdm(CONFIG["sigmas"], desc="信噪比", leave=False):
            # 生成数据
            X, y, beta_true = generate_experiment1(sigma=sigma)
            n, p = X.shape
            ...
            for method_name, method_config in tqdm(CONFIG["methods"].items(), desc="方法", leave=False):
                try:
                    ...
                    # 预测（使用最佳lambda）
                    # 直接获取最佳系数和截距，避免predict函数的类型检查问题
                    beta_pred = fit.coefs[fit.best_idx]
                    intercept = fit.intercept[fit.best_idx]
                    y_pred = X @ beta_pred + intercept
    ...
    result_dir = "result/exp_001"
    os.makedirs(result_dir, exist_ok=True)

    output_path = os.path.join(result_dir, "experiment_001_results.csv")
    ...
    # 保存汇总结果
    summary_path = os.path.join(result_dir, "experiment_001_summary.csv")
...
if __name__ == "__main__":
    # 命令行参数解析
    parser = argparse.ArgumentParser(description="实验1：高维成对相关稀疏回归")
