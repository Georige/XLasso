...
class DataGenerator:
    ...
    - experiment1: Exp1 - pairwise correlation 0.5, first 20 vars β=1.0 (pairwise alias)
    - experiment2: Exp2 - AR(1) ρ=0.8, odd-indexed first 20 vars β=1.0 (ar1 alias)
    - experiment3: Exp3 - AR(1) ρ=0.8, binomial with offset (experiment2 + offset)
    - experiment4: Exp4 - Opposite-sign twin variables with correlation (twin alias)
    - experiment5: Exp5 - Perfect Masking trap (experiment4 + orthogonalization)
    - experiment6: Exp6 - The Decoy Trap
    - experiment7: Exp7 - AR(1) Sign Avalanche
    """
    ...
    def generate(
        ...
        """
        if correlation_type in ("pairwise", "experiment1"):
            return self._generate_experiment1(n_samples, n_features, n_nonzero, sigma, family, rho)
        elif correlation_type in ("ar1", "experiment2"):
    ...
    def _generate_experiment1(self, n, p, k, sigma, family, rho) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Generate experiment 1 data: pairwise correlation, first 20 vars β=1.0.

        n=300, p=500, X~N(0,Σ) with Σ_ij=0.5
        first 20 variables β=1.0, remaining 480 variables β=0
        """
        cov = np.full((p, p), rho)
    ...
    def _generate_experiment2(self, n, p, k, sigma, family, rho) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Generate experiment 2 data: AR(1) correlation, odd-indexed first 20 vars β=1.0.

        n=300, p=500, X~N(0,Σ) with Σ_ij=0.8^|i-j|
        odd-indexed first 20 variables β=1.0 (j=1,3,5,...,39)
        """
        cov = np.zeros((p, p))
        for i in range(p):
            for j in range(p):
    ...
    def _generate_experiment3(self, n, p, k, sigma, family, rho) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def _generate_experiment5(self, n, p, sigma, family, rho) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Generate experiment 5 data: Perfect Masking trap.

        n=300, p=1000, 10 pairs twin variables with ρ=0.8
        β_{2t-1}=2.0, β_{2t}=-2.5, remaining 980 variables are independent noise
        Core challenge: designed to make marginal correlation of true signals zero
        """
        X = self.rng.randn(n, p)
        beta_true = np.zeros(p)
        ...
        # Perfect Masking: orthogonalize twin variables to zero out marginal correlations
        ...
        for i in range(10):
            ...
            # Make v1 orthogonal to y (zero marginal correlation)
